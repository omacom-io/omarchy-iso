...
def arch_install_system(ctx: InstallContext) -> None:
    ...
    with arch.open_installer(config, ctx.target, silent=True) as installer:
        ...
        if pre_mounted:
            ...
            _write_lvm_mkinitcpio_dropin(ctx)
        else:
            installer.genfstab()
...
def _blkid_type(device: str) -> str:
    fstype = capture_identifier(
        ["blkid", "-s", "TYPE", "-o", "value", device], f"the filesystem type of {device}"
    )
    if not fstype:
        raise RuntimeError(f"blkid returned no filesystem type for {device}")
    return fstype
...
def _write_pre_mounted_fstab(ctx: InstallContext) -> None:
    ...
    # An adopted /home is a volume of its own, so it replaces the @home line
    # rather than joining it. Writing both would mount two filesystems at the
    # same point and hide the one the owner asked to keep.
    if home_device := storage.get("home_device"):
        home_uuid = _blkid_uuid(home_device)
        home_type = _blkid_type(home_device)
        lines.append(f"UUID={home_uuid}  /home                  {home_type}  defaults,noatime  0 2")
    else:
        lines.append(f"UUID={btrfs_uuid}  /home                  btrfs  {btrfs_opts}@home   0 0")
    ...
    if swap_device := storage.get("swap_device"):
        swap_uuid = _blkid_uuid(swap_device)
        lines.append(f"UUID={swap_uuid}  none                   swap   defaults                0 0")

    lines.append("")
    (ctx.target / "etc" / "fstab").write_text("\n".join(lines))


# mkinitcpio sources /etc/mkinitcpio.conf.d in lexical order, and
# omarchy-settings ships omarchy_hooks.conf, which *assigns* HOOKS=(...) rather
# than appending to it. A 99- prefix — the convention the provisioning-key
# drop-in uses — sorts before that file and is silently overwritten; that
# drop-in survives only because it appends to FILES. Hence zz-.
#
# lvm2 goes after block (the PV must be readable) and before encrypt (the LUKS
# container lives on a logical volume, so the group is assembled first).
LVM_MKINITCPIO_DROPIN = "zz-omarchy-lvm.conf"


def _write_lvm_mkinitcpio_dropin(ctx: InstallContext) -> None:
    storage = _storage_intent(ctx)
    if not storage.get("lvm_volume_group"):
        return

    dropin = ctx.target / "etc/mkinitcpio.conf.d" / LVM_MKINITCPIO_DROPIN
    dropin.parent.mkdir(parents=True, exist_ok=True)
    dropin.write_text(
        "# Root lives on a logical volume; without lvm2 the initramfs cannot\n"
        "# assemble the volume group and never finds it.\n"
        "_omarchy_lvm_hooks=()\n"
        'for _omarchy_hook in "${HOOKS[@]}"; do\n'
        '  if [[ $_omarchy_hook == "encrypt" || $_omarchy_hook == "filesystems" ]] &&\n'
        '    [[ " ${_omarchy_lvm_hooks[*]} " != *" lvm2 "* ]]; then\n'
        '    _omarchy_lvm_hooks+=("lvm2")\n'
        "  fi\n"
        '  _omarchy_lvm_hooks+=("$_omarchy_hook")\n'
        "done\n"
        'HOOKS=("${_omarchy_lvm_hooks[@]}")\n'
        "unset _omarchy_lvm_hooks _omarchy_hook\n"
    )
...
def _validate_pre_mounted_filesystems(ctx: InstallContext) -> None:
    ...
    if home_device := storage.get("home_device"):
        home_uuid = _blkid_uuid(home_device)
        if home_uuid not in fstab_text:
            raise RuntimeError(f"{fstab} missing home UUID {home_uuid} for {home_device}")
    if swap_device := storage.get("swap_device"):
        swap_uuid = _blkid_uuid(swap_device)
        if swap_uuid not in fstab_text:
            raise RuntimeError(f"{fstab} missing swap UUID {swap_uuid} for {swap_device}")

    if storage.get("lvm_volume_group"):
        dropin = ctx.target / "etc/mkinitcpio.conf.d" / LVM_MKINITCPIO_DROPIN
        if not dropin.exists():
            raise RuntimeError(f"{dropin} missing; the initramfs would not assemble the volume group")
