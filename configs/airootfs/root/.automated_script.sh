...
install_base_system() {
  ...
  # Workaround for archinstall 4.2 regression in sync_log_to_install_medium:
  # `self.target / absolute_logfile` drops self.target because the RHS is
  # absolute, so Python 3.14 Path.copy() raises EINVAL (source == target).
  sed -i 's|logfile_target = self\.target / absolute_logfile$|logfile_target = self.target / absolute_logfile.relative_to("/")|' \
