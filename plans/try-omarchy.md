...
2.5 GiB in the cowspace tmpfs plus the running session (~0.8 GiB with a browser closed). That is too much for a 4 GiB machine as plain tmpfs, so the session turns on the same compressed swap an installed system runs: `zram-generator` is in the try set and `omarchy-settings` — already in the live root — ships the `zram-size = ram, zstd` config, so a `daemon-reload` and `systemctl start systemd-zram-setup@zram0` give the overlay's pages somewhere to be reclaimed to at roughly 3:1. Gate on `MemTotal ≥ 4 GiB`; below that, explain and return to the greeter. Prefetch is page cache and is reclaimed under pressure.
