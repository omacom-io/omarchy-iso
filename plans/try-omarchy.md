...
2.5 GiB in the cowspace tmpfs plus the running session (~0.8 GiB with a browser closed). archiso caps that tmpfs at half of physical RAM, and no amount of swap raises a tmpfs size cap, so the floor is a **nominal 8 GiB machine** — `omarchy-try` compares `MemTotal` against 7 GiB, because an 8 GiB machine reports ~7.6–7.9 GiB after kernel reservations, and below that explains and returns to the greeter. Then, before anything is installed, `omarchy-try-setup` checks the remounted overlay's free space against the set's installed size (written at build time to `usr/share/omarchy-iso/try-installed-kib`, plus 25% headroom) and refuses with real numbers rather than letting pacman fail mid-transaction. Compressed swap comes up before the install rather than after: `zram-generator` (1 MiB) is installed first, and `omarchy-settings` — already in the live root — ships the `zram-size = ram, zstd` config an installed system runs, so a `daemon-reload` and `systemctl start systemd-zram-setup@zram0` give the overlay's pages somewhere to be reclaimed to at roughly 3:1. Prefetch is page cache and is reclaimed under pressure.
...
2. **Grow the overlay.** `mount -o remount,size=50% /run/archiso/cowspace`. tmpfs resizes live; the mount point is the one archiso created. Then check its free space against the set's installed size (`try-installed-kib`, +25%) and refuse with numbers if it cannot hold it; then install `zram-generator` and start zram so the overlay's pages are reclaimable before the install fills it.
